...
const EditItem: React.FC<{
  ...
  const [image, setImage] = useState<File | "">(data?.picture || "");
  ...
  const handleSubmit = async (e: any, { resetForm }: any) => {
    try {
      ...
      console.log("resp edititem", response);
  ...
  useEffect(() => console.log("ini dataa", data), [data]);
  //   useEffect(() => {
  //     if (ref.current) {
  //       ref.current.value = String(image.name);
  //     }
  //   }, [image]);
    ...
    isVisible && (
      <div className="flex fixed inset-0 z-10 bg-opacity-30 bg-slate-700 backdrop-blur-sm justify-center items-center">
        ...
        <div className="flex flex-col gap-3 shadow rounded-lg min-w-[300px] max-h-[500px] p-3 bg-base-100 relative overflow-hidden">
          ...
          <h3 className="font-bold text-secondary">Edit Item</h3>
          <Formik
            ...
            initialValues={{
              ...
              qty: data?.qty || 0,
              id_unit:
                unitList.find((item) => item.name === data?.unit)?.id || "",
